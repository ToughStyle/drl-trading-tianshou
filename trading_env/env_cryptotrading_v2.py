...
class CryptoTradingEnv(gym.Env):
	...
	def __init__(
		...
		self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(
			self.state[self.crypto_dim + 1: self.crypto_dim * 2 + 1]), np.array(
	...
	def _sell_share(self, index, action):
		if self.state[index + 1] > 0:
			...
			self.cum_sell_cost += (
						self.state[index + self.crypto_dim + 1] * sell_num_shares * self.sell_cost_pct[index])
			self.sell_trades += 1
		else:
			sell_amount = 0
		...
		if terminal:
			...
			if self.timestamp > self.eval_time_interval and self.risk_control:
				short_term_reward = np.log(self.portfolio_values_memory[-1] / self.portfolio_values_memory[
					- self.eval_time_interval]) * self.alpha * self.eval_time_interval / self.data_granularity
	...
	def step(self, actions):
		...
		self.portfolio_value = self._calculate_portfolio_value(self.state[0], np.array(
			self.state[(self.crypto_dim + 1): (self.crypto_dim * 2 + 1)]), np.array(self.state[1: 1 + self.crypto_dim]))
	...
	def reset(self):
		...
		self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(
			self.state[self.crypto_dim + 1: 2 * self.crypto_dim + 1]), np.array(self.num_crypto_shares))
		self.cum_buy_cost = 0
