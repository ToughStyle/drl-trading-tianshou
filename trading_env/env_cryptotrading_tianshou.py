...
class CryptoTradingEnv(gym.Env):
	...
	def __init__(
		...
		self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(action_space_dim,))  # type: ignore # action space
		self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(state_space_dim,))  # type: ignore # observation space
		...
		self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(
			self.state[self.state_interval * self.crypto_dim + 1: (self.state_interval + 1) * self.crypto_dim + 1]), np.array(self.num_crypto_shares))  # current timestamp portfolio value
	...
	def _reward_setting(self, terminal):
		...
		if terminal:
			...
			portfolio_value = self.portfolio_values_memory[-2] if len(self.portfolio_values_memory) > 1 else self.max_portfolio_value
			reward = np.log(self.portfolio_values_memory[-1] / portfolio_value)
	...
	def step(self, actions):
		...
		if self.terminal:
			...
			self.portfolio_value = self._calculate_portfolio_value(self.state[0], np.array(
				self.state[(self.state_interval * self.crypto_dim + 1): ((self.state_interval + 1) * self.crypto_dim + 1)]), np.array(self.state[1: 1 + self.crypto_dim]))
	...
	def reset(self):
		...
		start_timestamp = self.state_interval - 1
		if self.is_training and self.cfg.random_reset:
			date_list = self.df.timestamp.unique()
			n = len(date_list)
			# end_timestamp = n - self.cfg.single_env_step
			end_timestamp = n - self.cfg.step_per_collect
			self.timestamp = np.random.randint(start_timestamp, end_timestamp)
		else:
			self.timestamp = start_timestamp
		if self.is_training:
			print(f'reset {self.timestamp = }\n')
		self.data = self.df.loc[self.timestamp - self.state_interval + 1: self.timestamp]
		self.state = self._initiate_state()
		self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(
			self.state[self.state_interval * self.crypto_dim + 1: (self.state_interval + 1) * self.crypto_dim + 1]), np.array(self.num_crypto_shares))
		self.cum_buy_cost = 0
